...
const CookieBanner = () => {
    ...
    if (!showBanner)
        return null;
    return (<main className={`${styles.wrapper} card`}>
      <section className={`${styles.banner} card`} role="dialog" aria-live="polite" aria-label="Cookie consent">
          ...
          <aside className={styles.controls}>
            <div className={styles.switch_div}>
              ...
              <label className={styles.switch}>
                <input type="checkbox" checked={stats} onChange={(e) => setStats(e.target.checked)} aria-label={cookie.enableStats}/>
                <span className={styles.slider}></span>
              </label>
            </div>
            <div className={styles.div}>
              <button className="button_invert" onClick={() => saveSelection({ statistics: stats })}>
                {cookie.save}
              </button>
