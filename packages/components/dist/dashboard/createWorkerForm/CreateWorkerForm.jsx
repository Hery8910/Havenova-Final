...
const CreateWorkerForm = () => {
    ...
    return (<section className={styles.container}>
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <select name="type" value={formData.employment?.type} onChange={(e) => handleNestedChange(e, 'employment')}>
          <option value="EMPLOYEE">Empleado</option>
          <option value="CONTRACTOR">Contratista</option>
        </select>

        <input type="date" name="startDate" value={formData.employment?.startDate} onChange={(e) => handleNestedChange(e, 'employment')}/>

        <input type="date" name="endDate" value={formData.employment?.endDate} onChange={(e) => handleNestedChange(e, 'employment')}/>

        <select name="type" value={formData.pay?.type} onChange={(e) => handleNestedChange(e, 'pay')}>
          <option value="HOURLY">Por hora</option>
          <option value="SALARIED">Salario mensual</option>
        </select>

        {formData.pay?.type === 'HOURLY' && (<input type="number" name="hourlyRate" placeholder="Tarifa por hora" value={formData.pay.hourlyRate || ''} onChange={(e) => handleNestedChange(e, 'pay')}/>)}

        {formData.pay?.type === 'SALARIED' && (<input type="number" name="monthlySalary" placeholder="Salario mensual" value={formData.pay.monthlySalary || ''} onChange={(e) => handleNestedChange(e, 'pay')}/>)}
      ...
      {alert && (<AlertPopup type={alert.type} title={alert.title} description={alert.description} onClose={() => setAlert(null)}/>)}
    </section>);
};
