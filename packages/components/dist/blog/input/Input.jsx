...
export default function Input({ heading, value, onChange, onBlur, placeholder = 'Blog Title', maxLength = 120, height, }) {
    ...
    return (<textarea ref={textareaRef} className={`${styles.titleInput} ${styles[`${heading}`]}`} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} rows={1} maxLength={maxLength} onInput={handleInput} style={height ? { height } : undefined}/>);
}
