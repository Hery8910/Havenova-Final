...
const ServicesPreview = ({ title, subtitle, description, items, cta, theme, }) => {
    ...
    return (<section className={styles.section} aria-labelledby="servicespreview-title">
      <div className={styles.bgWrapper} aria-hidden="true">
        <Image src={bgSrc} alt="" fill sizes="100vw" loading="lazy" fetchPriority="auto" className={styles.backgroundImage}/>
      </div>
      ...
      <ul className={styles.ul}>
        {items.map((item, idx) => (<li className={styles.li} key={`${idx}-${item.title}`}>
            <figure className={styles.figure}>
              <Image className={styles.icon} src={item.image.src} alt="" width={100} height={100} loading="lazy" decoding="async" sizes="100px"/>
            </figure>
