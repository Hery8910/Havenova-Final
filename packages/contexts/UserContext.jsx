...
// --- Utils ---
function getPersistentGuestAvatar() {
...
function getUserFromStorage() {
    ...
    try {
        ...
        if (typeof parsed.isLogged !== 'boolean')
            parsed.isLogged = parsed.role !== 'guest';
...
export const DashboardProvider = ({ children }) => {
    ...
    const sessionCallbackRef = React.useRef(null);
    // Carga inicial
    useEffect(() => {
        if (!clientId)
            return;
    ...
    // --- REFRESH USER ---
    const refreshUser = useCallback(async (onSessionExpired) => {
        setLoading(true);
        ...
        try {
            ...
            const fallback = localUser
                ? { ...guestBase, ...localUser, isLogged: false, role: 'guest' }
                : guestBase;
            setUser(fallback);
    ...
    const registerSessionCallback = useCallback((cb) => {
        sessionCallbackRef.current = cb;
    }, []);
    const updateUserLanguage = useCallback(async (newLang) => {
        // Invitado → solo local
