...
export function parseCookiePrefs(value) {
    if (!value)
        return null;
    try {
        const parts = value.split('&').reduce((acc, pair) => {
...
export function setCookie(name, value, days = 180) {
    ...
    document.cookie = `${name}=${value}; Path=/; SameSite=Lax; Expires=${expires};${isSecure ? ' Secure;' : ''}`;
}
...
export function getPrefsFromLocalStorage() {
...
export function savePrefsToLocalStorage(prefs) {
    if (typeof localStorage === 'undefined')
        return;
...
export function shouldRePrompt(existing) {
    if (!existing)
        return true;
...
export function eraseCookie(name, path = '/', domains) {
    if (typeof document === 'undefined')
        return;
    const base = `${name}=; Expires=Thu, 01 Jan 1970 00:00:01 GMT; Path=${path};`;
    const candidates = domains && domains.length > 0 ? domains : [location.hostname, `.${location.hostname}`];
